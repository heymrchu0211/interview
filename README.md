...
```
두번째 방법은 위와 같이 inline 혹은 inline-block 요소들의 margin을 음수값으로 적용 하는 것입니다. (네거티브 마진 이라고 불립니다 )  margin-right: -6px 라고 적용하게 되면 오른쪽 마진은 -6px이라는 의미인데요. 이렇게 하면 요소들 사이의 간격을 강제로 줄일 수 있습니다.<br><br>
...
```
세번째 방법은 해당 요소들의 부모 요소의 font-size를 위와 같이 0으로 만들면 간격이 사라지게 만들 수 있습니다. 그런데 해당 요소들이 부모 요소의 font-size를 상속 받아서 해당 요소들까지도 font-size가 0이 되버립니다. 그래서 해당 요소들에게 font-size를 따로 지정 해줘야 합니다. 하지만 font-size가 0인 요소는 화면낭독기에서 읽어주질 않기 때문에 이 세번째 방법은 웹접근성 측면에선 적절하지 않은 방법입니다.
...
브라우저의 렌더링 엔진은 웹문서가 로드가 되면 모든 html요소와 그 요소들에 있는 속성,텍스트들을 모두 각각의 객체로 만들고 이러한 객체들은 서로 부자관계로 연결이 되면서 그 모습이 마치 나무의 뿌리와 가지같아서 tree구조라고 부릅니다.<br> 
...
## 웹퍼블리셔는 어떤 역할을 하는 직군이라고 생각 하시나요?

디자이너와 개발자의 중간역할을 하는게 일반적인 웹퍼블리셔의 역할이라고 생각합니다. 디자이너에게 디자인 시안을 전달받고 그 디자인 시안을 토대로 웹표준과 웹접근성을 준수하면서 html,css 그리고 자바스크립트를 활용하여 웹에 표현하는 일을 합니다. 프론트단이 백엔드단과 연동이 되는 작업을 하기 바로 이전 단계를 작업하는 것이 웹퍼블리셔의 역할이라고 할 수 있습니다.<br><br>

그런데 회사마다 웹퍼블리셔의 역할을 각기 다르게 보는 것 같습니다. 채용공고를 보면 디자인과 퍼블리싱을 겸하는 경우도 있고 또는 제가 말씀드린 것처럼 디자이너와 개발자의 중간역할인 경우도 있고 아니면 퍼블리셔(프론트개발자) 라고 표기하여 프론트개발자와 별다른 구분이 없이 한데 묶어서 같은 직군으로 보는 회사도 있는 것 같습니다.<br><br>

역할이 딱 정해져 있는 것이 아니라 어떠한 상황에 있느냐에 따라 역할도 조금씩 달라질 것이라고 생각합니다.

## 웹퍼블리셔로 일하고 싶은 이유가 뭔가요?

처음에는 그저 디자인 시안을 웹페이지에 표현하는 일에 흥미를 느껴 시작하게 되었는데요.<br> 
그냥 혼자서만 클론 코딩해보는 것에 그치지 않고 실제로 고객들이 이용하는 웹사이트를 만드는 과정에 나도 참여하고 싶다는 생각이 들었고<br> 
실제로 그러한 결과물을 만들게 된다면 참 뿌듯하고 기쁠 것 같다는 생각이 들었습니다.

## 본인은 어떤 성격인가요?

무엇이든간에 항상 생각이 많은 성격입니다. 생각이 많다는건 신중하다는 장점이 될 수도 있지만  때로는 잡생각이 많아져서 지금 당장 해야할 일을 빨리 시작하지 못한다는 단점도 있습니다.<br>
그러한 단점을 극복하기 위해서 우선순위 목록을 만들고 그것을 지속적으로 꺼내보면서 쓸데없는 생각은 줄이고 당장에 해야 될 일에 더 집중하기 위해 애쓰고 있습니다.

## 미래에 대한 계힉 혹은 목표가 있나요?

현재는 웹퍼블리셔로서 일을 하기 위해 구직활동을 하고 있지만 장기적으로 봤을때는 프론트개발자를 지향하는 방향으로 가고 싶습니다. 제가 만약 웹퍼블리셔로 일을 한다고 하더라도 혼자서만 일하는게 아니고 개발단계에서 어떤식으로 일이 진행되는지를 알아야 원활한 협업이 이루어질 수 있을꺼라 생각하기 때문입니다. 그리고 개인적으로도 프론트개발을 공부하고 싶은 욕심이 있습니다.

## 실제 근무환경에서 가장 중요한 요소가 무엇이라고 생각하시나요?

실제 근무환경에서 가장 중요한 요소는 협업이라고 생각합니다.<br> 
